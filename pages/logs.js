...
const Logs = () => {
  ...
  useEffect(async () => {
    let [ipAddresses, dhcpData] = await Promise.all([
      axios.get('/api/rocksetDataIP'),
      axios.get('/api/rocksetDataDHCP'),
    ]);

    setIpAddressData(ipAddresses);
    setDhcpData(dhcpData);
    console.log(ipAddresses);
    console.log(dhcpData);
  }, []);
  ...
  const [lengthobj, setlengthobj] = useState(0);

  const [arrayObjsForData, setArrayObjsForData] = useState([{ x: '1', y: 1 }]);

  // state for dhcp message and date
  const [rdatamessage, setrdatamessage] = useState([]);
  const [rdatemessageObj, setrdatemessageObj] = useState([]);
  ...
  let reduceList = function (countryListToObject) {
    return new Promise((resolve, reject) => {
      if (countryListToObject == undefined) {
        ...
        const map = countryListToObject.reduce(
  ...
  */
  ...
  return (
    <div className="chart__container">
      <h1>Logs</h1>
      <button onClick={increment}>add +1 count</button>
      <p>This is a data point {counter} </p>

      <div className="chart__wrapper">
        <div className="pie_chart">
          <svg viewBox="0 0 250 350" className="svg">
            <VictorySharedEvents
              events={[
                {
                  childName: ['pie'],
